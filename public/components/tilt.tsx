...
import type { HTMLProps, ReactNode, RefObject } from "preact/compat";
...
/**
 * Tilt effect component, inspired by Apple's tvOS effect.
 *
 * Modified from `@jdion/tilt-react` for use in Preact.
 */
export class Tilt extends Component<Props, State> {
  ...
  getValues = (e: MouseEvent | TouchEvent) => {
    const x =
      (e instanceof TouchEvent ? e.touches[0].clientX : e.clientX - this.left) /
      this.width;
    const y =
      (e instanceof TouchEvent ? e.touches[0].clientY : e.clientY - this.top) /
      this.height;
